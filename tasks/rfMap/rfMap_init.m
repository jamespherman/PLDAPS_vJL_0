...
% Each stim type also gets a per-channel browser uifigure window, with
% tile content matched to the regime (image+line for spatial-RF
% accumulators; image-only with sequential colormap for the
% checkerboard F1-amplitude grid).
if p.trVarsInit.useRippleSTA
    switch p.init.stimType
        case 'checkerboard'
            ...
            p.init.staBrowser = initCheckerboardChannelBrowser( ...
                p.trVarsInit.nChannels, ...
                p.init.checkInfo.nCheckSize, ...
                p.init.checkInfo.nContrast, ...
                p.trVarsInit.checkSizesDva, ...
                p.trVarsInit.checkContrasts);
        otherwise
