...
function p = upsertSuccessfulTrial(p, goodTrialIdx, attemptTrialIdx, ...
        ...
        rtMs, chosenTargetID, chosenSide, chosenRewardMs, previousRewardMs, ...
        previousTrialType, previousChoseHighReward, ...
...
validSideChoice = isChoice & isfinite(chosenSide) & ismember(chosenSide, [1 2]);
stats.pRightChoice = safeMean(double(chosenSide(validSideChoice) == 1));
stats.nRightChoice = sum(validSideChoice);
...
statusLinesRight = { ...
    ...
    htmlLine('P(right choice)', probabilityString( ...
