...
switch p.stim.targLocationPreset
    case 'grid'
        ...
        % Defines a grid where each x/y intersection will be a target 
        % location stroes all X and Y positions in a list, and into the 
        % p.stim substruct.
        ...
        % make sure that a taget does not appear at the fixation point by 
        % removing the fixation point from the list of target locations:
        ptrToFp     = find(xx == p.trVarsInit.fixDegX & ...
            yy == p.trVarsInit.fixDegY);
        xx(ptrToFp) = [];
        yy(ptrToFp) = [];
    ...
    case {'ring', 'nRing'}
        ...
        for iR = 1:nRings
            ...
            for ii = 1:(p.stim.ringTargNumber(iR))
                xx(iT) = p.stim.ringRadius(iR) * ...
                    cosd(p.stim.ringBaseAngle(iR) + (ii-1) * ita);
                yy(iT) = p.stim.ringRadius(iR) * ...
                    sind(p.stim.ringBaseAngle(iR) + (ii-1) * ita);
                iT = iT + 1;
            end
        end
