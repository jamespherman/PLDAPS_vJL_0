...
% if we're using the trials array, define the number of trials left in the
% current "block" one way, otherwise do it another way.
if p.trVars.setTargLocViaTrialArray
    p.status.trialsLeftInBlock = nnz(p.status.trialsArrayRowsPossible);
else
    p.status.trialsLeftInBlock  = p.stim.nTargetLocations - ...
        (p.status.iTarget-1);
end
