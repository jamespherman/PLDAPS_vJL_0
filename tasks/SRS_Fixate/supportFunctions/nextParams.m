...
if p.status.CurrentBlockNumber == 0
    disp('Choose randm FirstBlock');
    % If First Block ; Choose randomly the type of the block
    x = randi(2);
    disp(x)
    disp('Prem Block')
    p.status.CurrentBlockType = x;
    p = ChangeBlock(p);
...
x = p.status.BlockRichMeanDuration +1000;
y = p.status.BlockRichMeanDuration +1000;
while (p.status.BlockRichMeanDuration < x || p.status.BlockRichMeanDuration< y) 
...
p.draw.T1_locPixY = p.draw.middleXY(2) - pds.deg2pix(p.trVars.T1_locDegY, p);            % negative because screen Y is inverted
...
p.draw.T2_locPixY = p.draw.middleXY(2) - pds.deg2pix(p.trVars.T2_locDegY, p); % negative because screen Y is inverted
