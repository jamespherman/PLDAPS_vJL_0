...
if isGoodTrial
    ...
    spatial = getCurrentSpatialChoiceMetrics(p, chosenTargetID);
    ...
    p = upsertSuccessfulTrial( ...
        ...
        choseHighSal, choseHighReward, rtMs, chosenTargetID, chosenSide, ...
        spatial.chosenVerticalSide, spatial.straddlesLR, spatial.straddlesUD, ...
        spatial.chosenHorizontalRank, spatial.chosenVerticalRank, ...
...
hasLeftStatus = isfield(p.draw.onlinePlotObj, 'statusTextLeft') && ...
    ishandle(p.draw.onlinePlotObj.statusTextLeft);
hasRightStatus = isfield(p.draw.onlinePlotObj, 'statusTextRight') && ...
    ishandle(p.draw.onlinePlotObj.statusTextRight);
hasLegacyStatus = isfield(p.draw.onlinePlotObj, 'statusText') && ...
    ishandle(p.draw.onlinePlotObj.statusText);

if hasLeftStatus || hasRightStatus || hasLegacyStatus
    [statusLinesLeft, statusLinesRight] = makeOnlineStatusLines( ...
        p, attemptTrialIdx, goodTrialIdx, isGoodTrial, rewardDiff, ...
        luminanceDiff, hueContrastDiff, ...
        onlineStats, currentTransition);

    if hasLeftStatus
        set(p.draw.onlinePlotObj.statusTextLeft, ...
            'String', statusLinesLeft, 'Value', 1);
    end
    if hasRightStatus
        set(p.draw.onlinePlotObj.statusTextRight, ...
            'String', statusLinesRight, 'Value', 1);
    end
    if hasLegacyStatus
        set(p.draw.onlinePlotObj.statusText, ...
            'String', [statusLinesLeft; {' '}; statusLinesRight], ...
            'Value', 1);
    end
end
...
function p = upsertSuccessfulTrial(p, goodTrialIdx, attemptTrialIdx, ...
        ...
        rtMs, chosenTargetID, chosenSide, chosenVerticalSide, straddlesLR, ...
        straddlesUD, chosenHorizontalRank, chosenVerticalRank, ...
        chosenRewardMs, previousRewardMs, previousTrialType, ...
        previousChoseHighReward, previousChoseHighSalience, switchedTarget)
...
op.chosenSide(idx) = chosenSide;
op.chosenVerticalSide(idx) = chosenVerticalSide;
op.straddlesLR(idx) = straddlesLR;
op.straddlesUD(idx) = straddlesUD;
op.chosenHorizontalRank(idx) = chosenHorizontalRank;
op.chosenVerticalRank(idx) = chosenVerticalRank;
...
rtMs = op.rtMs;
chosenSide = op.chosenSide;
chosenVerticalSide = op.chosenVerticalSide;
straddlesLR = op.straddlesLR;
straddlesUD = op.straddlesUD;
chosenHorizontalRank = op.chosenHorizontalRank;
chosenVerticalRank = op.chosenVerticalRank;
...
% Absolute hemifield preference must be conditioned on availability. A trial
% with two left targets contains no possible right-hemifield choice and is
% therefore excluded from P(right hemi). Relative rightmost/uppermost choice
% remains defined on same-hemifield trials and is reported separately.
validRightHemifield = isChoice & straddlesLR == 1 & ...
    isfinite(chosenSide) & ismember(chosenSide, [1 2]);
stats.pRightChoice = safeMean(double(chosenSide(validRightHemifield) == 1));
stats.nRightChoice = sum(validRightHemifield);

validRightmost = isChoice & isfinite(chosenHorizontalRank) & ...
    ismember(chosenHorizontalRank, [1 2]);
stats.pRightmostChoice = safeMean(double(chosenHorizontalRank(validRightmost) == 1));
stats.nRightmostChoice = sum(validRightmost);

validUpperHemifield = isChoice & straddlesUD == 1 & ...
    isfinite(chosenVerticalSide) & ismember(chosenVerticalSide, [1 2]);
stats.pUpperChoice = safeMean(double(chosenVerticalSide(validUpperHemifield) == 1));
stats.nUpperChoice = sum(validUpperHemifield);

validUppermost = isChoice & isfinite(chosenVerticalRank) & ...
    ismember(chosenVerticalRank, [1 2]);
stats.pUppermostChoice = safeMean(double(chosenVerticalRank(validUppermost) == 1));
stats.nUppermostChoice = sum(validUppermost);

validIdentityChoice = isChoice & isfinite(op.chosenTargetID) & ...
    ismember(op.chosenTargetID, [1 2]);
stats.pT1Choice = safeMean(double(op.chosenTargetID(validIdentityChoice) == 1));
stats.nT1Choice = sum(validIdentityChoice);
...
if isfield(p, 'draw') && isfield(p.draw, 'directRgbCalibration') && ...
        isstruct(p.draw.directRgbCalibration) && ...
        getNumericField(p.trVars, 'salienceType', NaN) == 3
...
elseif isfield(p, 'draw') && isfield(p.draw, 'clut') && ...
...
if ~isfinite(backgroundCdM2) && isfield(p, 'draw') && ...
    ...
    backgroundCdM2 = getNumericField( ...
        p.draw.directRgbCalibration, 'backgroundMeasuredCdM2', NaN);
end
if ~isfinite(backgroundCdM2) && isfield(p, 'draw') && ...
        isfield(p.draw, 'clut')
    backgroundCdM2 = getNumericField( ...
...
% SRS_mooving keeps the base task's chosenSide as an INTERNAL schedule slot
% for compatibility with reward delivery. Online spatial behavior must use
% the actual hemifield generated by the randomized target coordinates.
if isfield(p.trData, 'chosenPhysicalSide')
    chosenSide = getNumericField(p.trData, 'chosenPhysicalSide', NaN);
else
    chosenSide = getNumericField(p.trData, 'chosenSide', NaN);
end

% chosenTargetID is always saved by the current SRS state machine. This
% fallback is only for legacy records and is deliberately conservative when
% two moving targets happen to occupy the same hemifield.
if ~any(chosenTargetID == [1 2]) && any(chosenSide == [1 2])
    T1Side = getNumericField(p.trVars, 'T1PhysicalSide', NaN);
    T2Side = getNumericField(p.trVars, 'T2PhysicalSide', NaN);
    if chosenSide == T1Side && chosenSide ~= T2Side
        chosenTargetID = 1;
    elseif chosenSide == T2Side && chosenSide ~= T1Side
        chosenTargetID = 2;
    end
end
...
function spatial = getCurrentSpatialChoiceMetrics(p, chosenTargetID)
%GETCURRENTSPATIALCHOICEMETRICS Absolute and relative moving-position choice.
%
% Absolute right/left is meaningful as a binary preference only on trials
% where the targets straddle the vertical meridian. Relative rightmost and
% uppermost are defined even when both targets occupy the same hemifield.

spatial = struct( ...
    'chosenVerticalSide', NaN, ...
    'straddlesLR', NaN, ...
    'straddlesUD', NaN, ...
    'chosenHorizontalRank', NaN, ...
    'chosenVerticalRank', NaN);

if isfield(p.trData, 'chosenVerticalSide')
    spatial.chosenVerticalSide = getNumericField( ...
        p.trData, 'chosenVerticalSide', NaN);
end
spatial.straddlesLR = getNumericField( ...
    p.trVars, 'movingTargetsStraddleLR', NaN);
spatial.straddlesUD = getNumericField( ...
    p.trVars, 'movingTargetsStraddleUD', NaN);

if isfield(p.trData, 'chosenHorizontalRank')
    spatial.chosenHorizontalRank = getNumericField( ...
        p.trData, 'chosenHorizontalRank', NaN);
end
if isfield(p.trData, 'chosenVerticalRank')
    spatial.chosenVerticalRank = getNumericField( ...
        p.trData, 'chosenVerticalRank', NaN);
end

% Conservative fallback for records created before the rank fields existed.
if any(chosenTargetID == [1 2])
    if ~any(spatial.chosenVerticalSide == [1 2])
        if chosenTargetID == 1
            spatial.chosenVerticalSide = getNumericField( ...
                p.trVars, 'T1VerticalSide', NaN);
        else
            spatial.chosenVerticalSide = getNumericField( ...
                p.trVars, 'T2VerticalSide', NaN);
        end
    end
    if ~any(spatial.chosenHorizontalRank == [1 2])
        rightmost = getNumericField(p.trVars, 'rightmostTargetID', 0);
        leftmost = getNumericField(p.trVars, 'leftmostTargetID', 0);
        if chosenTargetID == rightmost && rightmost ~= 0
            spatial.chosenHorizontalRank = 1;
        elseif chosenTargetID == leftmost && leftmost ~= 0
            spatial.chosenHorizontalRank = 2;
        end
    end
    if ~any(spatial.chosenVerticalRank == [1 2])
        uppermost = getNumericField(p.trVars, 'uppermostTargetID', 0);
        lowermost = getNumericField(p.trVars, 'lowermostTargetID', 0);
        if chosenTargetID == uppermost && uppermost ~= 0
            spatial.chosenVerticalRank = 1;
        elseif chosenTargetID == lowermost && lowermost ~= 0
            spatial.chosenVerticalRank = 2;
        end
    end
end
...
function [statusLinesLeft, statusLinesRight] = makeOnlineStatusLines(p, attemptTrialIdx, ...
...
displayModeString = getDisplayModeString(p);
directRgbT1 = rgbTripletString(getNumericVectorField( ...
    p.trVars, 'T1_colorRGB255', [NaN NaN NaN]));
directRgbT2 = rgbTripletString(getNumericVectorField( ...
    p.trVars, 'T2_colorRGB255', [NaN NaN NaN]));
directMeasuredMean = getNumericField( ...
    p.trVars, 'DirectRgbPairMeasuredMeanCdM2', NaN);
directDesiredMean = getNumericField( ...
...
correctionEnabled = logical(getNumericField( ...
    p.trVars, 'correctionTrial', 0));
correctionActive = logical(getNumericField( ...
    p.status, 'correctionTrialActive', 0));
correctionRow = getNumericField(p.status, 'correctionTrialRow', NaN);
correctionRep = getNumericField( ...
    p.status, 'correctionTrialRepetition', 0);
correctionMax = getNumericField( ...
    p.trVars, 'correctionTrialMaxRepetition', 15);
correctionOutcome = getStringField( ...
...
statusLinesLeft = { ...
    ...
    htmlLine('Moving ecc / min sep', sprintf('%.1f deg / %.1f deg', ...
        getNumericField(p.trVars, 'movingTargetEccDeg', NaN), ...
        getNumericField(p.trVars, 'movingTargetMinSeparationDeg', NaN)), '#000000'); ...
    htmlLine('T1 position', movingPositionString( ...
        getNumericField(p.trVars, 'T1PhysicalSide', NaN), ...
        getNumericField(p.trVars, 'movingT1AngleDeg', NaN)), '#000000'); ...
    htmlLine('T2 position', movingPositionString( ...
        getNumericField(p.trVars, 'T2PhysicalSide', NaN), ...
        getNumericField(p.trVars, 'movingT2AngleDeg', NaN)), '#000000'); ...
    htmlLine('Rich target', targetAndSideString( ...
        getHighRewardTargetID(p), ...
        getNumericField(p.status, 'highRewardPhysicalSide', NaN)), '#000000'); ...
    htmlLine('High-salience target', targetAndSideString( ...
        getHighSalienceTargetID(p), ...
        getNumericField(p.status, 'highSaliencePhysicalSide', NaN)), '#000000'); ...
    ...
    ' '; ...
    htmlSection('CORRECTION TRIAL', '#9c5700'); ...
    htmlLine('Enabled', yesNoString(correctionEnabled), '#000000'); ...
    htmlLine('Active', yesNoString(correctionActive), '#000000'); ...
    htmlLine('Forced row', valueToString(correctionRow), '#000000'); ...
    htmlLine('Repetition', sprintf('%d / %d', ...
        round(correctionRep), round(correctionMax)), '#000000'); ...
    htmlLine('Last outcome', correctionOutcome, '#000000'); ...
    htmlLine('Exact condition saved', yesNoString(logical(getNumericField( ...
        p.status, 'correctionTrialSnapshotValid', 0))), '#000000'); ...
    htmlLine('Reduce RIGHT reward', yesNoString(logical(getNumericField( ...
        p.trVars, 'correctionReduceRightReward', 0))), '#000000'); ...
    htmlLine('RIGHT reward multiplier', formatNumber(getNumericField( ...
        p.trVars, 'correctionRightRewardMultiplier', NaN), '%.3f'), '#000000'); ...
    htmlLine('RIGHT reward original / applied', sprintf('%s / %s ms', ...
        formatNumber(getNumericField(p.trVars, ...
        'correctionOriginalRightRewardMs', NaN), '%.0f'), ...
        formatNumber(getNumericField(p.trVars, ...
        'correctionRightRewardAppliedMs', NaN), '%.0f')), '#000000'); ...
    htmlLine('Triggered / succeeded / max', sprintf('%d / %d / %d', ...
        round(getNumericField(p.status, 'correctionTrialTriggerCount', 0)), ...
        round(getNumericField(p.status, 'correctionTrialSuccessCount', 0)), ...
        round(getNumericField(p.status, 'correctionTrialMaxReachedCount', 0))), '#000000') ...
    };

statusLinesRight = { ...
    htmlTitle('STIMULUS / BEHAVIOR', '#1f4e79'); ...
    htmlSection('DISPLAY', '#5b6573'); ...
    htmlLine('Display mode', displayModeString, '#000000'); ...
    htmlLine('T1 / T2 RGB', sprintf('%s / %s', ...
        directRgbT1, directRgbT2), '#000000'); ...
    htmlLine('T1 / T2 red level', sprintf('%s / %s', ...
        valueToString(getNumericField(p.trVars, 'T1_redLevel', NaN)), ...
        valueToString(getNumericField(p.trVars, 'T2_redLevel', NaN))), '#000000'); ...
    ' '; ...
    htmlSection('MEASURED LUMINANCE', '#8064a2'); ...
    htmlLine('Background', backgroundLuminanceString(bgCdM2, bgDkl, ...
        getNumericField(p.trVars, 'salienceType', NaN)), '#000000'); ...
    ...
    htmlLine('Desired / measured mean', sprintf('%s / %s cd/m^2', ...
        formatNumber(directDesiredMean, '%.3f'), ...
        formatNumber(directMeasuredMean, '%.3f')), '#000000'); ...
    ' '; ...
    ...
    htmlLine('P(T1 identity)', probabilityString( ...
        stats.pT1Choice, stats.nT1Choice), '#1f4e79'); ...
    htmlLine('P(right hemi | L/R available)', probabilityString( ...
        stats.pRightChoice, stats.nRightChoice), '#7030a0'); ...
    htmlLine('P(rightmost target)', probabilityString( ...
        stats.pRightmostChoice, stats.nRightmostChoice), '#7030a0'); ...
    htmlLine('P(upper hemi | U/D available)', probabilityString( ...
        stats.pUpperChoice, stats.nUpperChoice), '#7f6000'); ...
    htmlLine('P(uppermost target)', probabilityString( ...
        stats.pUppermostChoice, stats.nUppermostChoice), '#7f6000'); ...
...
function txt = movingPositionString(side, angleDeg)

if isfinite(angleDeg)
    txt = sprintf('%s @ %.1f deg', sideName(side), angleDeg);
else
    txt = sideName(side);
end
...
function txt = getDisplayModeString(p)
%GETDISPLAYMODESTRING Human-readable display backend.
...
function value = getNumericVectorField(s, fieldName, defaultValue)
value = defaultValue;
if isstruct(s) && isfield(s, fieldName)
    candidate = s.(fieldName);
    if isnumeric(candidate)
        value = reshape(double(candidate), 1, []);
    end
end
end

function txt = rgbTripletString(rgb)
...
function txt = backgroundLuminanceString(cdM2, dklValue, salienceType)
if salienceType == 3
    txt = sprintf('%s cd/m^2  (RGB [0 0 0])', ...
        formatNumber(cdM2, '%.3f'));
else
    txt = sprintf('%s cd/m^2  (DKL %s)', ...
        formatNumber(cdM2, '%.1f'), formatNumber(dklValue, '%.3f'));
end
end
