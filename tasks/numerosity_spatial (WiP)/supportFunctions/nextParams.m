...
% For spatial_numerosity, we use a red fixation dot to tell the monkey
% which task it is doing
...
% Saving final matrices into p.trVars in case that data is needed for analysis
p.trVars.stim1_final = stim1_color;
p.trVars.stim2_final = stim2_color;
...
if p.trVars.numDots == 1
    % If presenting only one dot, set stimTwoDur to 0 so it doesn't appear
    p.trVars.timeStimTwoOnset       = p.trVars.timeStimOneOnset;
    p.trVars.stimTwoDur             = 0;
    p.trVars.timeStimTwoOffset      = p.trVars.timeStimTwoOnset + p.trVars.stimTwoDur;

elseif p.trVars.numDots == 2
    % If presenting two dots, set timing to same as stimOne so they appear
    % simultaneously
    p.trVars.timeStimTwoOnset       = p.trVars.timeStimOneOnset;
