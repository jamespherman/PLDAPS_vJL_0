...
for i = 1:p.trVars.nPatches
    % if this isn't the "1st" stimulus patch, we need to offset the
    % values we're using to index the array holding the check
    % color/intensity, calculate that offset here:
    if i > 1
        offsetVal = max(p.stim.stimArray{i - 1}(:)) + 1;
    else
        offsetVal = p.draw.nColors;
    end
    p.stim.stimArray{i}  = (reshape(unindex(hw.*arrayScale(unscaledIm, p.trVars.boxSizePix)), p.stim.patchDiamPix, p.stim.patchDiamPix) - 2 + offsetVal).*hw + (~hw)*p.draw.color.background;
    p.draw.stimTex{i}    = Screen('MakeTexture', p.draw.window, p.stim.stimArray{i});
end

% if the cued stimulus is being presented in location 2, flip the order of
% stimtex HACK ALERT
if p.stim.cueSide == 2
