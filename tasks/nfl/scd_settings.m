...
p.init.rigConfigFile     = which('rigConfig_rigB_20180802.m'); % rig config file has subject/rig-specific details (eg distance from screen)
...
p.rig.joyVoltageMax     = 2.5;
p.rig.joyVoltageMin     = 0.0034;
...
p.init.exptType         = 'full_noNoEmptyCue';  % Which experiment are we running? The full version with all trial types? The single-stimulus-only version? Something else?
...
p.trVarsInit.satDelta            = 0.1;      % color saturation
p.trVarsInit.lumDelta            = 0.1;      % luminance
p.trVarsInit.hueDelta            = 15;       % hue (color angle)
p.trVarsInit.stimLocElev         = 30;   % Stimulus location (angle of elevation).
p.trVarsInit.stimLocEcc          = 10;   % Stimulus location (eccentricity in degrees).
p.trVarsInit.stimRadius          = 6;    % aperture radius in deg
...
p.trVarsInit.cueDur                  = 0.2;      % Duration of ring presentaiton.
p.trVarsInit.cue2StimItvl            = 0.5;      % time between ring offset and motion onset (stimulus onset asynchrony).
...
p.trVarsInit.initSpeed                = (pi/4);   % initial motion magniutde
p.trVarsInit.initCtrst                = 0.2;      % initial contrast
p.trVarsInit.initOrient               = 30;       % initial orientation
p.trVarsInit.initFreq                 = 0.2;      % initial spatial frequency (cycles per degree)
p.trVarsInit.initSat                  = 0.4;      % initial color saturation
p.trVarsInit.initLum                  = 0;        % initial luminance
p.trVarsInit.initHue                  = 90;       % initial hue (color angle)
p.trVarsInit.boxSizePix               = 32;       % diameter of each "check" in pixels
...
p.trVarsInit.lumVar                   = 0.025;    % variability in luminance
p.trVarsInit.satVar                   = 0.05;     % variability in saturation

% stimulus vars that get set in nextParams:
...
p.draw.fixWinWidthDeg       = 4;        % fixation window width in degrees
p.draw.fixWinHeightDeg      = 4;        % fixation window height in degrees
...
p.draw.color.gridMajor  = p.draw.clutIdx.expGrey70_subBg;               % grid line CLUT index
...
p.init.strobeList = {...
    ...
    'p.trVars.stimLocEcc',                                      'cueRadius'; ...
    'p.trVars.cueElevDeg',                                      'cueTheta'; ...
