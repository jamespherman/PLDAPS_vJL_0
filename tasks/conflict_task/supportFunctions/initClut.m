...
% initClut
%
% Builds the single, static CLUT for the Conflict Task experiment.
% Adapted from gSac_4factors - uses only bullseye stimuli with DKL colors.
...
% Generate and insert the Grayscale Ramp (for compatibility)
...
for i = 1:length(idx_fields)
    ...
    if contains(field_name, '_subBg')
        ...
        subBg_rows(end+1) = index_to_update + 1;
    end
end
