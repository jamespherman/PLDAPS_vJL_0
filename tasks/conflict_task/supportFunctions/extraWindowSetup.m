...
% Get delta-t values from trial structure (set in initTrialStructure.m)
deltaTValues = p.init.deltaTValues;
xLimMargin = 25;  % margin on each side of the data range
xLimMin = min(deltaTValues) - xLimMargin;
xLimMax = max(deltaTValues) + xLimMargin;
...
% Add reference lines
plot(p.draw.tachAxes, [xLimMin xLimMax], [0.5 0.5], '--', 'Color', [0.5 0.5 0.5], ...
...
% Add text annotations explaining the axes (positioned relative to axis limits)
textPosLeft = xLimMin + 15;
textPosRight = xLimMax - 65;
text(p.draw.tachAxes, textPosLeft, 0.95, 'Stimuli before Go', ...
    'FontSize', 9, 'Color', [0.4 0.4 0.4]);
text(p.draw.tachAxes, textPosRight, 0.95, 'Stimuli after Go', ...
    'FontSize', 9, 'Color', [0.4 0.4 0.4]);
