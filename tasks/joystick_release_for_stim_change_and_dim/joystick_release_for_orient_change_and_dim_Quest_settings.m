function p = joystick_release_for_orient_change_and_dim_Quest_settings
...
p.init.exptType         = 'joystick_release_for_stim_dim_and_orient_change_quest';  % Which experiment are we running? <- IMPORTANT FOR TRIAL STRUCTURE CHOICE
...
p.trVarsInit.cue2StimIntvl           = 0.5;      % The value of this variable is updated before each trial but we set it to the max here just in case.
p.trVarsInit.stim2ChgIntvl           = 1.0;      % minimum time between stimulus onset and change.
p.trVarsInit.chgWinDur               = 2.5;      % time window during which a change is possible.
...
p.init.quest.tGuess                  = 35;        % Initial guess for threshold (degrees)
p.init.quest.tGuessSd                = 25;        % Standard deviation of the initial guess
p.init.quest.pThreshold              = 0.75;     % Target performance level for threshold
p.init.quest.beta                    = 5;      % Steepness of the Weibull function
p.init.quest.delta                   = 0.01;     % Lapse rate (guess rate is gamma)
p.init.quest.gamma                   = 0.01;      % Guess rate (chance performance)
p.init.quest.grain                   = 0.01;     % Discretization of the stimulus space
p.init.quest.range                   = 200;        % Range of possible orientation changes
p.init.quest.minVal                  = 1;        % Minimum possible orientation change (must be > 0)
p.init.quest.maxVal                  = 90;       % Maximum possible orientation change
...
p.init.strobeList = fliplr({...                 
    ...
    'p.trVars.isOptoStimTrial',                                                                                 'optoStimTrial'
    });
end
