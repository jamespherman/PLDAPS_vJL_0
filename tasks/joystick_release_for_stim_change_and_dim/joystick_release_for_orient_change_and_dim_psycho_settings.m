function p = joystick_release_for_orient_change_and_dim_psycho_settings
%  p = joystick_release_for_orient_change_and_dim_psycho_settings
%  
%  Version of orient change and dim cued where orientDelta randomly switches
%  between 4 different values. Used to create a psychometric curve to
%  estimate optimal difficulty for the monkey so he actually needs to make
%  use of the cue.
%
...
p.init.exptType         = 'joystick_release_for_stim_dim_and_orient_change_psycho';  % Which experiment are we running? <- IMPORTANT FOR TRIAL STRUCTURE CHOICE
...
p.init.trDataInitList = {...
    'p.trData.eyeX',                    '[]'; ...   % horizontal eye position
    'p.trData.eyeY',                    '[]'; ...   % vertical eye position
    'p.trData.eyeP',                    '[]'; ...   % pupil diameter from Eyelink
    'p.trData.eyeT',                    '[]'; ...   % time vector for eye position / pupil size / joystick voltage
    'p.trData.joyV',                    '[]'; ...   % joystick voltage
    'p.trData.dInValues',               '[]'; ...   % values of digital inputs
    'p.trData.dInTimes',                '[]'; ...   % times of digital inputs
    'p.trData.spikeTimes',              '[]'; ...   % times of spikes retrieved from recording system
    'p.trData.eventTimes',              '[]'; ...   % times of strobed events retrieved from recording system
    'p.trData.eventValues',             '[]'; ...   % values of strobed events retrieved from recording system
    'p.trData.onlineEyeX',              '[]'; ...   % horizontal eye position sampled once per frame
    'p.trData.onlineEyeY',              '[]'; ...   % vertical eye position sampled once per frame
