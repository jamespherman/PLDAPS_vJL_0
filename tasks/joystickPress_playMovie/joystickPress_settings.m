function p = joystickPress_settings
%  p = joystickPress_settings
%
...
p.audio.pcPlayback      = true;  % do we want audio playback from psychtoolbox PC?
...
p.status.joyPressReq                = 0; % how long was the monkey required to hold down the joystick on the last trial?
...
% variables related to moive playback: (note, we need to figure out having
% a directory of movie files that we draw from at random, for now we're
% hardcoding a single movie file to play).
p.draw.movie.hdr            = 0;
p.draw.movie.movieOptions   = [];
p.draw.movie.moviecount     = 1;
p.draw.movie.coolstuff      = 0;
p.draw.movie.rate           = 1;
p.draw.movie.shader         = [];
p.draw.movie.pixelFormat    = [];
p.draw.movie.blocking       = 1;
p.draw.movie.preloadsecs    = [];
...
% that are all '.'.
movieDirList                = dir(p.draw.movie.movieDir);
movieDirList                = {movieDirList(:).name};
movieDirList(cellfun(@(x)all(x=='.'), movieDirList)) = [];
nMovies                     = length(movieDirList);

% choose a file at random to play:
p.draw.movie.movieName      = [p.draw.movie.movieDir filesep ...
    movieDirList{randi(nMovies)}];
