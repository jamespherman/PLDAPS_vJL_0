function p = fix_present_squares_run(p)
%   p = fix_present_squares_run(p)
%
...
if p.trData.timing.fixAq > 0
    ...
    if p.trVars.presentationCounter > p.trVars.numPresentations
	    % This if statement is mostly just to prevent the code from 
	    % checking stimOn with presentationCounter higher than the indices go
    ...
    elseif (p.trData.timing.stimOn(p.trVars.presentationCounter) ~= -1 && ...
            ...
            p.trVars.stimIsOn   = false;
            p.trData.timing.stimOff(p.trVars.presentationCounter)  = timeNow;
