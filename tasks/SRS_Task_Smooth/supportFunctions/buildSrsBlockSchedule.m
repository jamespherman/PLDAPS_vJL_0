...
%
% Training blocks contain two schedule phases:
%   1. Mixed T1-only and T2-only instruction trials
%   2. Two-target congruent/conflict choice trials
%
% T1-only and T2-only rows are randomized together. Target side remains
% counterbalanced. A failed instruction trial stays eligible and is sampled
% again later, while choice trials cannot begin until every instruction row
...
%% Define schedule phases
% All single-target identities share phase 1 so T1-only and T2-only trials
% can interleave. Choice trials use phase 2 and remain locked until every
% instruction row has been completed successfully.
if useTraining
    p.status.FirstSingleTargetID = 0;
    p.status.SecondSingleTargetID = 0;
    p.status.SingleTargetsMixed = true;
    instructionPhase = 1;
else
    p.status.FirstSingleTargetID = 0;
    p.status.SecondSingleTargetID = 0;
    p.status.SingleTargetsMixed = false;
    instructionPhase = NaN;
end
...
if useTraining
    if randomizeSides
        ...
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 1, 1, 2, 0, instructionPhase, nT1Right); % T1 only, T1 right
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 1, 2, 1, 0, instructionPhase, nT1Left);  % T1 only, T1 left
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 2, 2, 1, 0, instructionPhase, nT2Right); % T2 only, T2 right
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 2, 1, 2, 0, instructionPhase, nT2Left);  % T2 only, T2 left
    else
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 1, 1, 2, 0, instructionPhase, nSingleT1); % T1 fixed right
        [conditionRows, conditionID] = addCondition(conditionRows, conditionID, cols, ...
            1, 2, 1, 2, 0, instructionPhase, nSingleT2); % T2 fixed left
    end
end
