...
%% ------------------------------------------------------------
% Smooth-hue SRS variant: reserved dynamic target slots
% ------------------------------------------------------------
% Continuous (Dubey-style) hue-contrast sampling draws arbitrary target
% hues per trial, so a precomputed hue bank is not used. Instead, two CLUT
% slots are reserved for T1 and T2. nextParams writes the exact per-trial
% DKL RGB into these rows (both exp and sub sides) and re-pushes the CLUT.
% Background hues remain the classic DKL 0/180 deg values above.

% Fixed DKL luminance and saturation shared by the background poles and
% both targets. Default to the classic values used for the hue banks.
smoothMeanLum = mean_lum;
smoothSatRad = satRad;
if isfield(p, 'trVarsInit')
    if isfield(p.trVarsInit, 'hueDklMeanLuminance')
        smoothMeanLum = p.trVarsInit.hueDklMeanLuminance;
    end
    if isfield(p.trVarsInit, 'hueDklSaturationRadius')
        smoothSatRad = p.trVarsInit.hueDklSaturationRadius;
    end
end

% Gamut safety across the FULL hue circle. Because targets can take any hue
% (background + offset, offset in [min,max], for backgrounds at 0 or 180
% deg), every hue on the circle must be in gamut at the chosen saturation.
% Iteratively shrink the saturation radius until the whole circle fits.
% (M_dkl2rgb is already declared global earlier in this function.)
circleHues = 0:359;
maxSatIter = 25;
satOK = false;
for iTry = 1:maxSatIter
    satOK = true;
    for iHue = 1:numel(circleHues)
        theta = circleHues(iHue);
        dklC = [smoothMeanLum; ...
            smoothSatRad * cosd(theta); ...
            smoothSatRad * sind(theta)];
        rawC = round((0.5 + M_dkl2rgb * dklC / 2) * 255);
        if any(rawC < 0) || any(rawC > 255)
            satOK = false;
            break
        end
    end
    ...
    smoothSatRad = smoothSatRad * 0.95;
end
if ~satOK
    warning(['Smooth-hue saturation radius could not be brought fully ', ...
        'into gamut after %d iterations (final satRad = %.4f).'], ...
        maxSatIter, smoothSatRad);
end

p.draw.clut.smoothHueMeanLuminance = smoothMeanLum;
p.draw.clut.smoothHueSaturationRadius = smoothSatRad;

% Reserved CLUT slots (0-based) for the two dynamically colored targets.
% Slots 232:255 are otherwise unused (filled with background) and do not
% overlap the red-luminance bank (200:231) or the classic/overlay entries.
p.draw.clutIdx.srsTargetT1 = 250;
p.draw.clutIdx.srsTargetT2 = 251;

if p.draw.clutIdx.srsTargetT1 < p.draw.clutIdx.redLumStart + redLumN || ...
        p.draw.clutIdx.srsTargetT2 < p.draw.clutIdx.redLumStart + redLumN
    error('Reserved SRS target CLUT slots overlap the red luminance bank.');
end

% Initialize the reserved rows to the background so they are defined before
% the first hue trial writes real target colors.
p.draw.clut.expCLUT(p.draw.clutIdx.srsTargetT1 + 1, :) = bgRGB;
p.draw.clut.subCLUT(p.draw.clutIdx.srsTargetT1 + 1, :) = bgRGB;
p.draw.clut.expCLUT(p.draw.clutIdx.srsTargetT2 + 1, :) = bgRGB;
p.draw.clut.subCLUT(p.draw.clutIdx.srsTargetT2 + 1, :) = bgRGB;
...
% the subject display in hue/contrast mode. This is required by srsSmooth_run.m
