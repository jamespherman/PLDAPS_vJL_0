...
fprintf('\n---------------------- BLOCK SCHEDULE ----------------------\n');
printField('Instruction schedule', instructionOrderString(p.status));
...
fprintf('\n--------------------- HUE CONTRAST -------------------------\n');
printField('Hue sampling mode', ...
    getField(p.trVars, 'hueSamplingMode', 'missing'));
printField('Background hue', formatScalar( ...
    getField(p.trVars, 'BackgroundHue', NaN), '%.1f deg'));
printField('High / low hue', sprintf('%s / %s', ...
    formatScalar(getField(p.trVars, 'HighSalienceHueDeg', NaN), '%.1f deg'), ...
    formatScalar(getField(p.trVars, 'LowSalienceHueDeg', NaN), '%.1f deg')));
printField('T1 / T2 hue', sprintf('%s / %s', ...
    formatScalar(getField(p.trVars, 'ActualHueT1', NaN), '%.1f deg'), ...
    formatScalar(getField(p.trVars, 'ActualHueT2', NaN), '%.1f deg')));
printField('T1 / T2 distance from bg', sprintf('%s / %s', ...
    formatScalar(getField(p.trVars, 'HueContrastT1', NaN), '%.1f deg'), ...
    formatScalar(getField(p.trVars, 'HueContrastT2', NaN), '%.1f deg')));
printField('Contrast diff T1-T2', formatScalar( ...
    getField(p.trVars, 'HueContrastDifferenceT1MinusT2', NaN), ...
    '%.1f deg'));
...
if getField(status, 'TotalInstructionTrialsPerBlock', 0) > 0
