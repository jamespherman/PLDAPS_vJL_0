...
% In continuous-hue mode the T1/T2 target colors are written into reserved
% CLUT slots by nextParams each trial. Push the rebuilt CLUT to the ViewPIXX
% now, before any drawing, so the targets show their exact per-trial hues.
if isfield(p.trVars, 'updateClutThisTrial') && p.trVars.updateClutThisTrial && ...
        isfield(p.draw, 'clut') && isfield(p.draw.clut, 'combinedClut')
    Datapixx('SetVideoClut', p.draw.clut.combinedClut);
end
