function p = gSac_settings_trialsArray
%  p = gSac_settings
%
%   gSac task
% =============
...
p.init.rigConfigFile     = which('rigConfig_rigB_20180712.m'); % rig config file has subject/rig-specific details (eg distance from screen)
...
p.init.taskActions{1} =  'dataToWorkspace.m';
...
p.trVarsInit.rewardDuration          = 0.35; % reward duration
...
p.trVarsInit.maxSacDurationToAccept  = 0.08; % this is the max duration of a saccades that we're willing to wait for. 
...
%% stimulus-specific:
% The 'stimulus' in this taks is for the targets.
% target locations are set via mouse location, gui, or appear on a grid.
% The following paramters set the grid range and bin size:
p.stim.gridMinX     = -20;
p.stim.gridMaxX     = 20;
p.stim.gridBinSizeX = 2;
p.stim.gridMinY     = -15;
p.stim.gridMaxY     = 15;
p.stim.gridBinSizeY = 2;
