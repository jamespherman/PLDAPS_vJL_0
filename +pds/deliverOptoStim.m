...
if p.trVars.optoIpiSec == 0
    ...
    nPulses = 1;
    p.trVars.optoStimDurSec = p.trVars.optoStimDurSec;
    dacWaveOutDur = p.trVars.optoStimDurSec + 2*p.rig.dp.dacPadDur;
else
...
% Start DAC schedule, strobe optoStim code, and mark optoStim delivery time
Datapixx('StartDacSchedule');  
Datapixx('RegWrRd');
p.init.strb.strobeNow(p.init.codes.optoStimOn);
p.trData.timing.optoStim = GetSecs - p.trData.timing.trialStartPTB;