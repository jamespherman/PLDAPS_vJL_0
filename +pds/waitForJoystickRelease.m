...
% get DAC status to check if reward schedule is running:
dacStatus = Datapixx('GetDacStatus');

% note the time
startTime = GetSecs;

% Check to see if the "run" button is pressed in the GUI. To do this we
% first retreive the graphics handle for the "run" button, then query its
% "value":
runButtonObj = findall(groot, 'Tag', 'runButton');
runButtonLogical = logical(runButtonObj.Value);

% if the joystick is depressed, enter and stay in the while loop until it's
% been released.
while (pds.joyHeld(p) && ~p.trVars.passJoy && runButtonLogical) || ...
        dacStatus.scheduleRunning
